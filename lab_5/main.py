...
from engine import engine, model, types, scene
        ...
        d = [
            self.__create_param_block(
                ...
                [
                    ...
                    self.__create_param_field(
                        ...
                        self.__create_double_spin_box(0, 2, 0.5),
                    ),
                ]
            ),
            ...
            self.__create_param_block(
                ...
                [
                    ...
                    self.__create_param_field(
                        'intensity', 'Интенсивность',
                        self.__create_double_spin_box(0, 2, 0),
                    ),
                ]
            ),
            ...
            self.__create_param_block(
                ...
                [
                    self.__create_param_field(
                        'intensity', 'Интенсивность',
                        self.__create_double_spin_box(0, 2, 0.3),
                    ),
                ]
            ),
...
class Canvas(QFrame):
    def __init__(self, parent: QWidget, _engine: engine.Engine, scene: scene.Scene, cylinder_color: types.Color) -> None:
        ...
        self._engine = _engine
        self._scene = scene
        self._cylinder_color = cylinder_color
    ...
    def paintEvent(self, _: QPaintEvent) -> None:
        ...
        self._cylinder_color.a = abs(math.sin(time.time() - self._first_frame_time))
...
class MainWindow(QWidget):
    def __init__(self) -> None:
        ...
        self._back_wall = scene.SceneObject(
            name='back-wall',
            scale=types.Vector3(3.0, 3.0, 3.0),
            rotation=types.Vector3(0, 0, 0),
            position=types.Vector3(0, 0, 6),
            mesh=model.load('./lab_5/stand.obj'),
        )

        self._cylinder_color = types.Color(0, 255, 0, 0.4)
        self._cylinder = scene.SceneObject(
            name='cylinder',
            scale=types.Vector3(1.0, 1.0, 1.0),
            rotation=types.Vector3(0, 0, 0),
            position=types.Vector3(0, 0, 5),
            mesh=cylinder(1.0, 2.0, 40, self._cylinder_color, 0.0),
        )

        self._scene = scene.Scene()
        self._scene.add_object(self._cylinder)
        self._scene.add_object(self._back_wall)
        self._scene.add_object(scene.AmbientLight('ambient-light', 0.3))
        self._scene.add_object(scene.PointLight('point-light', 0.5, types.Vector3(0.0, 0.0, 0.0)))
        self._scene.add_object(scene.DirectionalLight('directional-light', 0.0, types.Vector3(0.0, 0.0, 0.0)))
    ...
    def __init_widgets(self, layout: QVBoxLayout) -> None:
        ...
        canvas_with_settings_layout.addWidget(Canvas(self, self._engine, self._scene, self._cylinder_color), 1)
